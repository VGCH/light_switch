...
const char api_js[] = R"rawliteral(
             <script>
      function api_genkey(){
         var api_gen = new XMLHttpRequest();
          ...
          if (api_gen.readyState === 4 && api_gen.status === 200) {
            ...
            // console.log('Данные успешно отправлены на сервер ' + channelL);
...
const char js[] = R"rawliteral(
      ...
      function server_time(){
         var worktime = new XMLHttpRequest();
          ...
          <script>
      function on_off(channelL){
         var power = new XMLHttpRequest();
             power.open('POST', '?page=power',true);
          ...
          if (power.readyState === 4 && power.status === 200) {
                ...
                document.getElementById("pbut"+channelL).innerHTML    = myArray[0];
                //document.getElementById("radios").innerHTML  = myArray[1];
                 }
              }
             console.log('Данные успешно отправлены на сервер ' + channelL);
             document.getElementById("pbut"+channelL).innerHTML  = "<center>Секундочку...</center>";
             power.send('switch=' + channelL);
           }
        </script>
                  <script>
